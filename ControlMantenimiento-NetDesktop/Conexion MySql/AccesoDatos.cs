   ...
   public class AccesoDatos
    {
      ...
      public static void IniciarBusqueda(string Tabla, string DatoBuscar, string Condicion)
      {
        try
        {
              Cn = new MySqlConnection(System.Configuration.ConfigurationSettings.AppSettings["Conexion"]);
      ...
      public static int ValidarTablaVacia(string Tabla)
      {
          try
          {
              using (Cn = new MySqlConnection(System.Configuration.ConfigurationSettings.AppSettings["Conexion"]))
              {
      ...
      public static ArrayList CargarListas(string Tabla, string Condicion)
        {
            try
            {
                ...
                using (Cn = new MySqlConnection(System.Configuration.ConfigurationSettings.AppSettings["Conexion"]))
                {
                    ...
                    while (sdr.Read())
                    {
                       arlLista.Add(new CargaCombosListas(sdr.GetValue(0).ToString(), sdr.GetValue(1).ToString()));
                    }
                    sdr.Close();                                                  
                    LiberarRecursos();
                }
